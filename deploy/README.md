...
Окончание смены с клиента приходит в UTC (`toISOString`); API переводит его в `APP_TIMEZONE` перед записью. После смены `.env` обязательно `config:clear`.

Типичный баг «в активной смене задачи есть, после завершения — нет»: `ended_at` сохранился как UTC wall-clock и оказался **раньше** `started_at` (Москва). Статистика схлопывала окно в ноль. Исправлено в `AppDateTime::shiftWindow()` + нормализация при `end`.
