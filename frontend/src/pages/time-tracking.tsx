...
import { ArrowDown, ArrowUp, ArrowUpDown, Clock, MessageSquare, MessageSquareOff } from 'lucide-react';
...
export function TimeTrackingPage() {
  ...
  // Own sidebar actions update immediately even before poll/broadcast arrives.
  const ownShiftKey = [
    ownShift?.id ?? 'none',
    ownShift?.status ?? '',
    ownShift?.endedAt ?? '',
  ...
  return (
    <div>
      <PageHeader
        title="Учет времени"
        description="Все рабочие смены сотрудников"
      />
                    ...
                    <td className="px-3 py-2.5">
                      {shift.comment?.trim() ? (
                        <span
                          ...
                          title={shift.comment}
                        >
                          <MessageSquare className="h-4 w-4" aria-label="Есть комментарий" />
                        </span>
                      ) : (
                        <span
                        ...
                        >
                          <MessageSquareOff className="h-4 w-4" aria-label="Нет комментария" />
                        </span>
                      )}
                    </td>
                  </tr>
